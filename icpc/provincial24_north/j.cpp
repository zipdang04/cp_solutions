...
bool check(Point a, Point b) {
    ...
    return x * x + y * y <= 4 * RADIUS * RADIUS;
}
...
main()
{
    ...
    FOR(int, i, 1, n) {
        ...
        if (p[i].y <= RADIUS) flow.addEdge(SOURCE, IN(i), 1);
        if (p[i].y >= w - RADIUS) flow.addEdge(OUT(i), SINK, 1);
    }
    FOR(int, i, 1, n) FOR(int, j, i + 1, n) {
        if (check(p[i], p[j]))
            flow.addEdge(OUT(i), IN(j), 1),
            flow.addEdge(OUT(j), IN(i), 1);
    }
    cout << flow.calculate();
}